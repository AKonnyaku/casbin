...
def main():
    ...
    for b in bench_data.get("benchmarks", []):
        ...
        # Also handle Go benchmark names
        name = re.sub(r"^Benchmark", "", name)
        name = re.sub(r"-\d+$", "", name)

        # value in ns/op (mean is in seconds)
        # Note: This script assumes input is compatible with pytest-benchmark JSON where 'stats' exists.
        # If feeding format_benchmark_data.py output, this will fail.
        # But we keep it for structure replication.
        if "stats" in b:
            value = b["stats"]["mean"] * 1e9
        else:
             # Fallback if we fed it formatted data?
             value = b.get("value", 0)
